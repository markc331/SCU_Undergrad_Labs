...
int main(int argc, char *argv[]) {
    ...
    {
        perror("Failure to bind server to the address endpoint");
        exit(1);
    }
    ...
    do {
        packet =  serverReceive(sockfd, (struct sockaddr *)&clienAddr, &addr_len, seqnum); //receive a packet and check through serverReceive()
