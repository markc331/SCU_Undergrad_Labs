...
// Description : checks the total copy time for both function and system calls
...
void check_copy_times(char* src_filename, char* dst_filename) {
	...
	func_start = clock(); // capture runtime of func_copy() using start clock, call the copy, end the clock
	func_copy(src_filename, dst_filename);
	func_end = clock();

	syscall_start = clock();// capture runtime of syscall_copy() using start clock, call the copy, end the clock
...
int main(int argc, char* argv[]) {
	...
	func_copy(argv[1], argv[2]);//get the source and destination files from the command line arguments
